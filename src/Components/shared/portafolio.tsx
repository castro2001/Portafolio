...
// import { Menu, X, Github, Linkedin, Mail, ExternalLink, Code2, Database, Palette, Moon, Sun, Globe, ChevronDown, Home, Briefcase, Wrench, MessageSquare, Image } from 'lucide-react';
...
//   const getLocation = () => {
//     return language === 'es' ? 'Guayaquil/Ecuador' : 'Guayaquil/Ecuador';
//   };
...
//       <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? (isDark ? 'bg-gray-900/80 backdrop-blur-lg shadow-lg border-b border-gray-800' : 'bg-white/80 backdrop-blur-lg shadow-lg border-b border-gray-200') : ''}`}>
...
//                 <button className={`flex items-center gap-2 px-3 py-2 rounded-full ${isDark ? 'bg-gray-800/50 hover:bg-gray-700' : 'bg-gray-100/80 hover:bg-white'} transition-all backdrop-blur-sm`}>
//                   <Globe size={16} />
//                   <span className="text-sm">{language.toUpperCase()}</span>
//                 </button>
//                 <div className={`absolute top-full right-0 mt-2 ${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-[120px]`}>
//                   <button 
//                     onClick={() => setLanguage('es')}
//                     className={`block w-full px-4 py-2 text-left text-sm hover:bg-blue-500 hover:text-white transition-colors ${language === 'es' ? 'bg-blue-500 text-white' : ''}`}
//                   >
//                     🇪🇸 Español
//                   </button>
//                   <button 
//                     onClick={() => setLanguage('en')}
//                     className={`block w-full px-4 py-2 text-left text-sm hover:bg-blue-500 hover:text-white transition-colors ${language === 'en' ? 'bg-blue-500 text-white' : ''}`}
//                   >
//                     🇺🇸 English
//                   </button>
//                 </div>
...
//                 className={`p-2 rounded-full transition-all duration-300 ${isDark ? 'bg-gray-800/50 hover:bg-gray-700' : 'bg-gray-100/80 hover:bg-white'}`}
...
//               {/* Clock */}
//               <div className={`px-4 py-2 rounded-full ${isDark ? 'bg-gray-800/50' : 'bg-gray-100/80'} backdrop-blur-sm font-mono text-sm`}>
//                 {formatTime()}
//               </div>
//             </div>
//           </div>
...
//       {/* Floating Dock Navigation - Bottom (Mobile & Tablet) */}
//       <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 md:hidden">
//         <div className={`flex items-center gap-2 ${isDark ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-lg rounded-full px-4 py-3 shadow-2xl border ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
//           {navItems.map((item, i) => {
...
//               >
//                 <Icon size={20} className={`transition-colors ${isActive ? 'text-white' : 'group-hover:text-blue-500'}`} />
//                 {isActive && (
//                   <span className="text-xs font-medium whitespace-nowrap animate-fade-in">{item.label}</span>
//                 )}
//               </a>
//             );
//           })}
//           <div className={`w-px h-8 ${isDark ? 'bg-gray-700' : 'bg-gray-300'} mx-1`}></div>
//           <a
//             href="#contact"
//             className={`flex flex-col items-center gap-1 px-3 py-2 rounded-full transition-all duration-300 ${
//               activeSection === 'contact' ? 'bg-blue-500 text-white' : 'text-blue-500'
//             }`}
//           >
//             <Mail size={20} />
//             {activeSection === 'contact' && (
//               <span className="text-xs font-medium whitespace-nowrap animate-fade-in">Contacto</span>
//             )}
//           </a>
//         </div>
//       </div>

//       {/* Tablet Vertical - Side Dock */}
//       <div className="hidden md:flex lg:hidden fixed left-6 top-1/2 -translate-y-1/2 z-40">
//         <div className={`flex flex-col items-center gap-2 ${isDark ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-lg rounded-full px-3 py-4 shadow-2xl border ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
//           {navItems.map((item, i) => {
//             const Icon = item.icon;
//             const isActive = activeSection === item.id;
//             return (
...
//                 title={item.label}
//               >
//                 <Icon size={22} className={`transition-colors ${isActive ? 'text-white' : 'group-hover:text-blue-500'}`} />
//                 {!isActive && (
//                   <span className={`absolute left-full ml-3 px-3 py-1 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white'} text-sm whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-lg`}>
//                     {item.label}
//                   </span>
//                 )}
//                 {isActive && (
//                   <span className={`absolute left-full ml-3 px-3 py-2 rounded-lg bg-blue-500 text-white text-sm font-medium whitespace-nowrap shadow-lg animate-fade-in`}>
//                     {item.label}
//                   </span>
//                 )}
//               </a>
//             );
//           })}
//           <div className={`w-8 h-px ${isDark ? 'bg-gray-700' : 'bg-gray-300'} my-2`}></div>
//           <a
//             href="#contact"
//             className={`group relative flex items-center justify-center p-3 rounded-full transition-all duration-300 ${
//               activeSection === 'contact'
//                 ? 'bg-blue-500 text-white scale-110'
//                 : isDark ? 'bg-blue-500/10 hover:bg-blue-500/20 text-blue-500' : 'bg-blue-50 hover:bg-blue-100 text-blue-500'
//             }`}
//             title="Contacto"
//           >
//             <Mail size={22} className={activeSection === 'contact' ? 'text-white' : ''} />
//             {activeSection === 'contact' && (
//               <span className={`absolute left-full ml-3 px-3 py-2 rounded-lg bg-blue-500 text-white text-sm font-medium whitespace-nowrap shadow-lg animate-fade-in`}>
//                 Contacto
//               </span>
...
//       {/* Desktop Large - Side Dock with Labels */}
//       <div className="hidden lg:flex fixed left-6 top-1/2 -translate-y-1/2 z-40">
//         <div className={`flex flex-col items-start gap-2 ${isDark ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-lg rounded-2xl px-4 py-4 shadow-2xl border ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
//           {navItems.map((item, i) => {
//             const Icon = item.icon;
//             return (
//               <a
//                 key={i}
//                 href={`#${item.id}`}
//                 className={`group flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 w-full ${isDark ? 'hover:bg-gray-800 active:bg-gray-700' : 'hover:bg-gray-100 active:bg-gray-200'}`}
//               >
...
//             );
//           })}
//           <div className={`w-full h-px ${isDark ? 'bg-gray-700' : 'bg-gray-300'} my-1`}></div>
//           <a
//             href="#contact"
//             className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 w-full text-blue-500 ${isDark ? 'bg-blue-500/10 hover:bg-blue-500/20' : 'bg-blue-50 hover:bg-blue-100'}`}
//           >
//             <Mail size={20} />
...
//       {/* Contact Section */}
//       <section id="contact" 
//       className={`py-20 px-4 ${isDark ? 'bg-gray-900/50' : 'bg-white'} 
//       relative`}>
